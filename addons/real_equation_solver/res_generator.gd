...
static func quadratic(...roots: Array) -> Array[float]:
	if roots.size() > 2:
		_ResLogger.format_error(_SCRIPT, quartic, "There cannot be more than 2 arguments", [])
		return []
	...
	if roots.is_empty(): 
		const LIMIT: int = 10
		# (x^2 + p) + q = 0
		var p: float = randi_range(-LIMIT, LIMIT)
		var q: float = randi_range(1, LIMIT)
		b = 2 * p
		c = p * p + q
	else:
...
static func cubic(...roots: Array) -> Array[float]:
	if roots.is_empty() or roots.size() > 3:
		_ResLogger.format_error(_SCRIPT, quartic, "There must be 1 to 3 arguments", [])
		return []
...
static func quartic(...roots: Array) -> Array[float]:
	...
	if roots.is_empty():
		...
		# (x^2 + px + q)^2 + r = 0
		var p: float = randi_range(-LIMIT, LIMIT)
		var q: float = randi_range(-LIMIT, LIMIT)
		var r: float = randi_range(1, LIMIT)
		b = 2 * p
		c = p * p + 2 * q
		d = 2 * p * q
		e = q * q + r
